...
from Doraemon import YouTube, app
from Doraemon.core.call import Dora
from Doraemon.misc import SUDOERS, db
from Doraemon.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from Doraemon.utils.decorators.language import languageCB
from Doraemon.utils.formatters import seconds_to_min
from Doraemon.utils.Inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from Doraemon.utils.Stream.autoclear import auto_clean
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        check = db.get(chat_id)
        txt = f"» ᴛʀᴀᴄᴋ ՏKIᑭᑭᗴᗪ ᗷY {mention} !"
        popped = None
        try:
            popped = check.pop(0)
            if popped:
